/*
  SG90 + ESP32-C3 SuperMini reactive clicker mock firmware

  Flow:
  - Simulates an "AI inference completed" signal every MOCK_INTERVAL_MS.
  - Blinks status LED while waiting.
  - On completion, moves SG90 from REST_ANGLE to PRESS_ANGLE briefly, then returns.

  Hardware defaults:
  - SG90 signal: GPIO4
  - Optional external LED: GPIO10 via 220-1k ohm resistor to GND
  - SG90 V+ must be separate 5V, GND shared with ESP32-C3.
...
// Mock completion cadence. Replace mockInferenceComplete() with real trigger later.
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("SG90 ESP32-C3 reactive clicker mock starting");
